...
CREATE TABLE `orders` (
  ...
  `checked_at`    DATETIME      DEFAULT NULL COMMENT '检票时间',
  `updated_at`    DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `notification` (
  ...
  `user_id`          BIGINT  NOT NULL COMMENT '用户ID,关联user表',
  `type`             ENUM('LOCK_SUCCESS','PAY_SUCCESS','TIMEOUT_CANCEL','REFUND_SUCCESS','SCHEDULE_CHANGE','SCREENING_REMINDER','ORDER_CANCELLED','EXPIRED','TASK_FAILURE') NOT NULL COMMENT '通知类型',
