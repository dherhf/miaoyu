...
function getSeatInfo(n: number) {
  ...
  if (n < 10) return { text: `紧张 ${n}席`, color: 'danger' as const, soldOut: false }
...
export default function SessionListCard({ data, onAction }: BaseCardProps<SessionListCardData>) {
  ...
  if (sessions.length === 0) {
    return <ErrorBlock status="empty" description="暂无符合条件的场次" />
  }
              ...
              <div key={s.id} style={S.item}>
                <div style={S.left}>
                  <div style={S.timeRow}>
                    ...
                    {cross && <Tag color="primary" fill="outline">{dateLabel}</Tag>}
                  </div>
                ...
                <div style={S.right}>
                  ...
                  <Tag color={seat.color} fill="outline">{seat.text}</Tag>
                </div>
                <div style={S.btnArea}>
                  <Button
                    size="small"
