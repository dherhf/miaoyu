...
export default function CinemaListCard({ data, onAction }: BaseCardProps<CinemaListCardData>) {
  ...
  if (cinemas.length === 0) {
    return <ErrorBlock status="empty" description="暂无符合条件的影院" />
  }
  return (
    <div style={S.wrap}>
      {cinemas.map((c) => (
          ...
          <div style={S.meta}>
            <div style={S.tags}>
              {c.facilities?.map((f, i) => (
                <Tag key={i} color={getFacilityColor(f)} fill="outline">{f}</Tag>
              ))}
            </div>
            {c.distance && <span style={S.dist}>距您 {c.distance}</span>}
          </div>
          <Button color="primary" block onClick={() => onAction(`选${c.name}`)}>
            选{c.name}
          </Button>
        </div>
      ))}
