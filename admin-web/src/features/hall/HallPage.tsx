...
import { Modal, Button, Tag, Space, Typography, Card, Form } from "antd";
...
export function HallPage() {
  ...
  const openEdit = async (hall: HallItem) => {
    setEditingHall(hall);
    let seats = hall.seats?.length
      ? hall.seats
      : generateSeats(hall.rowCount, hall.colCount);
    try {
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      const rowCnt = values.seats.length
        ? Math.max(...values.seats.map((s) => s.row))
        : 0;
      const colCnt = values.seats.length
        ? Math.max(...values.seats.map((s) => s.col))
        : 0;
      const savePayload = { ...values, rowCount: rowCnt, colCount: colCnt };
      if (editingHall) {
        const hasSchedule = schedules.some(
          (s) =>
            String(s.hallId) === String(editingHall.id) &&
            s.status !== "cancelled" &&
            s.status !== "ended",
        );
        if (
          hasSchedule &&
          (rowCnt !== editingHall.rowCount || colCnt !== editingHall.colCount)
        ) {
  ...
  const handleDelete = (hall: HallItem) => {
    const hasSchedule = schedules.some(
      (s) =>
        String(s.hallId) === String(hall.id) &&
        s.status !== "cancelled" &&
        s.status !== "ended",
    );
  ...
  const columns: ProColumns<HallItem>[] = [
    {
      ...
      render: (_, record) => {
        ...
        return (
          <Space size={12}>
            <div
              className={styles.hallIconBox}
              style={{ background: `${typeItem?.color || "blue"}10` }}
            >
              <AppstoreOutlined
                style={{
                  fontSize: 20,
                  color: `${typeItem?.color || "#1677ff"}`,
                }}
              />
            </div>
            <div>
              ...
              <div>
                <Typography.Text
                  type="secondary"
                  className={styles.hallTypeLabel}
                >
                  {typeItem?.label}
                </Typography.Text>
              </div>
    ...
    },
    {
      ...
      valueEnum: Object.fromEntries(
        HALL_TYPES.map((t) => [t.value, { text: t.label }]),
      ),
    },
    {
      title: "座位布局",
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size={4}>
          ...
          <Typography.Text className={styles.availableSeatsText}>
            {record.totalSeats}
          </Typography.Text>
        </Space>
      ),
    },
    {
      ...
      render: (_, record) => {
        const cfg =
          HALL_STATUS_LABELS[record.status] || HALL_STATUS_LABELS.active;
    ...
    <div className={styles.pageWrap}>
      ...
      {!selectedCinemaId && (
        <div className={styles.selectCinemaBox}>
          ...
          <Typography.Title level={5} className={styles.selectCinemaTitle}>
            请选择影院
          </Typography.Title>
          <Typography.Text type="secondary" className={styles.selectCinemaHint}>
            选择对应影院后管理影厅
          </Typography.Text>
          <div className={styles.cinemaGrid}>
            {cinemas.map((c) => (
              <Card
                ...
                key={c.id}
                onClick={() => {
                  setSelectedCinemaId(String(c.id));
                  navigate(`/halls?cinemaId=${c.id}`);
                }}
              ...
              >
                ...
                <div className={styles.cinemaCardHalls}>
                  <TeamOutlined
                    style={{ fontSize: 12 }}
                    className={styles.cinemaCardHallIcon}
                  />{" "}
                  {c.hallCount} 个影厅
                </div>
              </Card>
